...
public class Demo {
	...
	private static void usage() {
		$.msgln("Usage:  java -jar pysonar-2.0-SNAPSHOT.jar <file-or-dir> <output-dir>");
		$.msgln("Example that generates an index for Python 2.7 standard library:");
		$.msgln(" java -jar pysonar-2.0-SNAPSHOT.jar /usr/lib/python2.7 ./html");
		System.exit(0);
	}
...
//        System.out.println("options: " + options.getOptionsMap());
	...
	private void makeOutputDir() {
		if (!OUTPUT_DIR.exists()) {
			OUTPUT_DIR.mkdirs();
			$.msgln("Created directory: " + OUTPUT_DIR.getAbsolutePath());
		}
	}
	...
	private void start(@NotNull String fileOrDir, Map<String, Object> options) throws Exception {
		...
		$.msgln("Loading and analyzing files");
		...
		for (String path : analyzer.getLoadedFiles()) {
			if (path.startsWith(rootPath)) {
				...
				try {
					...
					$.msgln("Failed to write: " + destPath);
				}
			}
		}
