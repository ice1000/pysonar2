...
public class Parser {
	...
	public Parser() {
		...
		if (python2Process != null) {
			$.msgln("started: " + PYTHON2_EXE);
		}

		if (python3Process != null) {
			$.msgln("started: " + PYTHON3_EXE);
		}
	}
	...
	public Node convert(Object o) {
		...
		if (type.equals("Assert")) {
			...
			Node msg = convert(map.get("msgln"));
		...
		if (type.equals("NameConstant")) {
			...
			if (value == null) {
				...
				$.msgln("[WARNING] NameConstant contains unrecognized value: " + value + ", please report issue");
				strVal = "";
			}
	...
	public Op convertOp(Object map) {
		...
		$.msgln("[please report] unsupported operator: " + type);
	...
	public Process startInterpreter(String pythonExe) {
		...
		try {
			...
			$.msgln("Failed to start: " + pythonExe);
			return null;
		}
		return p;
	}
